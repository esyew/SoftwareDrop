...
public class MainDriver {
	...
	// This runs instead of all of the code being in Main, makes it a little easier to understand and handle. 
	
    private static void downloadJSON() {
		 try {
				String url = "https://milestone-bdd7e-default-rtdb.firebaseio.com/inventory.json"; 
		            downloadUsingNIO(url, "firebaseDL.json");
		            
		            downloadUsingStream(url, "firebaseDL.json");
		            // Good Download
		        } catch (IOException e) {
		            e.printStackTrace();
		            // Bad download
		            System.out.println("Error downloading firebase JSON file, please fix or replace with backup json\nto inventory.json");
